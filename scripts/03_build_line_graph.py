...
def build_line_graph(df: pd.DataFrame) -> Tuple[nx.Graph, pd.DataFrame]:
    ...
    weights = edge_df.groupby(
        ["source", "target"]).size().reset_index(name="weight")

    G = nx.from_pandas_edgelist(
        weights, source="source", target="target", edge_attr="weight")
...
if __name__ == "__main__":
    ...
    if not IN_FILE.exists():
        logging.error(
            f"Input file {IN_FILE} not found. Ensure station data is fetched first.")
        exit(1)
    ...
    logging.info(
        f"Graph successfully built with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges.")
